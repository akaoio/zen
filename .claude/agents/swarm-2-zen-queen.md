...
Created: 2025-08-07T07:21:08.890Z
...
TASK_FILE=$(node task.js create swarm-2-zen-queen "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-2-zen-queen "What I'm doing" target-files)
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-2-zen-queen "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-2-zen-queen "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-2-zen-queen")
    ...
    local coord_task=$(node task.js create swarm-2-zen-queen "Cross-swarm coordination: $coordination_issue" "coordination")
