...
Created: 2025-08-07T07:21:09.298Z
...
TASK_FILE=$(node task.js create swarm-3-zen-worker-stdlib "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-3-zen-worker-stdlib "What I'm doing" target-files)
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-3-zen-worker-stdlib "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-3-zen-worker-stdlib "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-3-zen-worker-stdlib`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-3-zen-worker-stdlib "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-3-zen-worker-stdlib "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-3-zen-worker-stdlib")
    ...
    local coord_task=$(node task.js create swarm-3-zen-worker-stdlib "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-3-zen-worker-stdlib "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-3-zen-worker-stdlib "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-3-zen-worker-stdlib "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-3-zen-worker-stdlib, part of swarm-3.
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-3-zen-worker-stdlib "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
