...
Created: 2025-08-07T07:21:09.611Z
...
TASK_FILE=$(node task.js create swarm-4-zen-worker-runtime "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-4-zen-worker-runtime "What I'm doing" target-files)
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-4-zen-worker-runtime "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-4-zen-worker-runtime "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-4-zen-worker-runtime`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-4-zen-worker-runtime "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-4-zen-worker-runtime "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-4-zen-worker-runtime")
    ...
    local coord_task=$(node task.js create swarm-4-zen-worker-runtime "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-4-zen-worker-runtime "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-4-zen-worker-runtime "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-4-zen-worker-runtime "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-4-zen-worker-runtime, part of swarm-4.
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-4-zen-worker-runtime "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
