...
Created: 2025-08-07T09:53:31.599Z
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-memory "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-2-zen-worker-memory "What I'm doing" target-files)
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-2-zen-worker-memory "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-2-zen-worker-memory "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-2-zen-worker-memory`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-2-zen-worker-memory "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-2-zen-worker-memory "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-2-zen-worker-memory")
    ...
    local coord_task=$(node task.js create swarm-2-zen-worker-memory "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-2-zen-worker-memory "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-2-zen-worker-memory "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-2-zen-worker-memory "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-2-zen-worker-memory, part of swarm-2.
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-2-zen-worker-memory "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
