...
export function setupRelayPex(zen, opts = {}) {
  ...
  // Use wss when a local SSL key is present OR when the public port is 443
  // (Cloudflare Tunnel / reverse proxy terminates TLS externally — no local cert needed).
  const scheme   = (key || annPort === 443) ? "wss" : "ws";
  ...
  const origin = domain ? `${scheme}://${domain}${portSuffix}/zen` : null;
