...
var BUCKET_COUNT  = 256;              // one bucket per XOR bit-position
...
function start(root) {
  ...
  // ── Learn peers from connect / disconnect ────────────────────────────────
  root.on("hi", function (peer) {
    this.to.next(peer);
    if (peer.pub && peer.url) { bucketAdd(peer.pub, peer.url); }
  });
