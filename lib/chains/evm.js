...
const _SECP_P = 0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2fn
...
function _pointAdd(a, b) {
    if (!a) return b
    if (!b) return a
    if (a.x === b.x) {
        if (_mod(a.y + b.y, _SECP_P) === 0n) return null
        const m = _mod((3n * a.x * a.x) * _modInv(2n * a.y, _SECP_P), _SECP_P)
        const x = _mod(m * m - 2n * a.x, _SECP_P)
        return { x, y: _mod(m * (a.x - x) - a.y, _SECP_P) }
    }
    const m = _mod((b.y - a.y) * _modInv(b.x - a.x, _SECP_P), _SECP_P)
    const x = _mod(m * m - a.x - b.x, _SECP_P)
    return { x, y: _mod(m * (a.x - x) - a.y, _SECP_P) }
}

function _pointMultiply(scalar, point) {
    let n = _mod(scalar, _SECP_N)
    let out = null
    let addend = point
    while (n > 0n) {
        if (n & 1n) out = _pointAdd(out, addend)
...
function _liftX(x, parity) {
    const y2 = _mod(x * x * x + 7n, _SECP_P)
    let y = _modPow(y2, (_SECP_P + 1n) >> 2n, _SECP_P)
    if ((y & 1n) !== parity) y = _SECP_P - y
    return y
}

function _recoverPubPoint(v, r, s, hashBytes) {
    const z = _mod(bytesToBigInt(hashBytes), _SECP_N)
    const R = { x: r, y: _liftX(r, BigInt(v) & 1n) }
    const rInv = _modInv(r, _SECP_N)
    const u1 = _mod((_SECP_N - _mod(z, _SECP_N)) * rInv, _SECP_N)
    const u2 = _mod(s * rInv, _SECP_N)
    const point = _pointAdd(_pointMultiply(u1, _SECP_G), _pointMultiply(u2, R))
    if (!point) throw new Error("recoverTypedDataAddress: recovery failed")
    return point
}

function _normalizeRecoverySignature(sig) {
    if (typeof sig === "string") {
        const bytes = hexToBytes(sig)
        if (bytes.length !== 65) throw new Error("recoverTypedDataAddress: signature must be 65 bytes")
        let v = bytes[64]
        if (v >= 27) v -= 27
        return {
    ...
    let v = typeof sig.v === "bigint" ? Number(sig.v) : Number(sig.v)
    if (v >= 27) v -= 27
    return {
        r: typeof sig.r === "bigint" ? sig.r : BigInt(sig.r),
        s: typeof sig.s === "bigint" ? sig.s : BigInt(sig.s),
        v,
    }
}
...
async function recoverTypedDataAddress(domain, types, value, sig, primaryType) {
    ...
    const { r, s, v } = _normalizeRecoverySignature(sig)
    const point = _recoverPubPoint(v, r, s, hashBytes)
    const pubBytes = concatBytes(bigIntToBytes(point.x, 32), bigIntToBytes(point.y, 32))
    const addrHash = await _keccak256(pubBytes)
    return checksumAddress("0x" + addrHash.slice(-40))
}
