...
function isDynamicType(type, components) {
    ...
    if (type === "tuple")                    // dynamic if any component is dynamic
...
function encodeParams(params) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        if (type === "tuple") {
            // Build inner encoded blob for the tuple components
            ...
            if (isDynamicType("tuple", components)) {
                // Dynamic tuple (contains bytes/string/array): pointer in head, data in tail
...
async function buildCalldata(sig, inputs, values) {
    const hashHex = await ZEN.hash(strToBytes(sig), null, null, {name: "keccak256", encode: "hex", input: "raw"})
    const sel = hexToBytes(hashHex).slice(0, 4)
    ...
    async getGasPrice() {
        ...
        try {
            ...
            if (block?.baseFeePerGas) {
                ...
                // EIP-1559: legacy gasPrice must be >= baseFee; add 20% headroom
                if (suggested < baseFee) return baseFee * 12n / 10n
            }
        } catch { /* non-EIP-1559 network, ignore */ }
        return suggested
    }
...
async function buildEventTopicMap(abi) {
    ...
    for (const item of abi) {
        ...
        const topic0  = "0x" + await ZEN.hash(strToBytes(sig), null, null, {name: "keccak256", encode: "hex", input: "raw"})
