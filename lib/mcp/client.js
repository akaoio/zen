...
export class ZenMcpClient {
  ...
  constructor(serverPub, zen, pair, opt = {}) {
    ...
    this._off = null; // mesh.onRelay unsubscribe handle
    this._closed = false;
  ...
  async ready() {
    ...
    this._off = mesh.onRelay(async ({ from, data }) => {
  ...
  async request(method, params, timeout) {
    ...
    return new Promise((resolve, reject) => {
      ...
      mesh.relay(this._serverPub, encrypted, { ttl: this._ttl });
    });
  }
  ...
  /**
   * Discover an MCP server's info from its well-known graph soul ~<serverPub>/mcp/info.
  ...
  static discover(serverPub, zen, timeout = 2000) {
    return new Promise((resolve) => {
      ...
      zen.get("~" + serverPub + "/mcp/info").once((v) => {
        clearTimeout(t);
        resolve(v ?? null);
      });
    });
