...
//
// The attach/boot split lets relay.service embed MCP directly (Gun/SEA pattern):
...
export async function boot() {
  ...
  try {
    ...
    if (hwIdentity) {
      ...
      zenOpt.pub = hwPub;
      // pid: AXE peer-dedup identity — must be deterministic but DIFFERENT from relay's pid
      // (relay uses hwPub as its pid; same pid triggers AXE self-connection drop)
      zenOpt.pid = await ZEN.hash({ data: hwPub + "/mcp" });
    }
  } catch (e) {
...
// Safe to call from relay.service (Gun/SEA pattern): no new ZEN instance created,
