...
const LOCAL_RELAY = "http://localhost:8420/zen";

// Try to bind port 8420. Returns the listening http.Server if successful,
...
function tryBindRelay() {
  return new Promise((resolve) => {
    ...
    server.listen(8420, () => resolve(server));
  });
}
...
export async function start() {
  ...
  //   - If a relay is already running at :8420 → connect as thin peer (rfs: false)
  //   - If no relay → become the relay: listen on :8420, use shared radata storage
  ...
  if (process.env.ZEN_PEERS) {
    ...
    zenOpt.axe = false; // disable axe to prevent localhost:8420 peer discovery
