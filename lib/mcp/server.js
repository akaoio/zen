...
// Read the relay port from the config file, defaulting to 8420.
function localRelayPort() {
  try {
    return parseInt(require("fs").readFileSync(join(xdg.config(), "port"), "utf8").trim(), 10) || 8420;
  } catch (_) {
    return 8420;
  }
}
...
export async function start() {
  ...
  // Single relay rule: only one relay per machine, one shared data path.
  //   - If a relay is already running → connect as thin peer (rfs: false)
  //   - If no relay → become the relay: listen on port, use shared radata storage
  // Either way, all data is stored at xdg.data() + "/radata" (rfs default).
  let zenOpt = {};
  if (process.env.ZEN_PEERS) {
    zenOpt.peers = process.env.ZEN_PEERS.split(",").filter(Boolean);
    // Use rfs default storage (xdg.data() + "/radata")
  } else if (process.env.NO_BOOTSTRAP === "1") {
    zenOpt.peers = []; // explicit empty → ZEN won't auto-add BOOT peers
    zenOpt.axe = false; // disable axe to prevent localhost peer discovery
    zenOpt.multicast = false; // disable UDP multicast peer discovery
    zenOpt.file = xdg.data() + "/mcp"; // isolated storage for tests
  } else {
    const boundServer = await tryBindRelay();
    if (boundServer) {
      // Port was free — we become the relay
      zenOpt.web = boundServer;
      // file not set → rfs uses xdg.data() + "/radata" (same path relay.js would use)
    } else {
      // Port already in use — relay is running, connect as thin peer.
      // When the relay uses TLS (cert.pem present), we must use https:// so ZEN
      // upgrades to wss://. Disable TLS cert validation for localhost: the relay
      // cert is domain-specific, not for 127.0.0.1, so validation would always fail.
  ...
  // Capture initial peers array before new ZEN() mutates zenOpt.peers into an object
  ...
  // Active subscriptions — sub_id → { off, from } (from = relay requester pub, null for stdio)
  const subStore = new Map();
  let subSeq = 0;
  function sendNotification(sub_id, soul, key, val, from) {
  ...
  function startRelayMode() {
    ...
    function publishStatus(ip4, ip6) {
      ...
      // Graph soul: stored as plain object — ZEN auth handles integrity.
      // No `timestamp` field: use HAM state (node._.>) for timing.
      const { timestamp: _ts, ...soulPayload } = payload;
  ...
  async function dispatchRelay(msg, from) {
    ...
    if (method === "tools/call") {
      ...
      try {
        const result = await call(name, args || {}, from);
        return { jsonrpc: "2.0", id, result: {
...
Useful for real-time agent-to-agent coordination without touching the graph.`,
  ...
  async function call(name, args, from) {
    // ── push ──────────────────────────────────────────────────────────────────
    if (name === "push") {
    ...
    if (name === "graph") {
      ...
      if (args.op === "subscribe") {
        ...
        node.on((val, key) => {
          sendNotification(sub_id, soul, key, val, from);
        });
        subStore.set(sub_id, { node, from });
        return { sub_id };
      }
