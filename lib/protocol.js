...
function zenPut(node, value, opts) {
...
async function decodeSignedMessage(val) {
  ...
  const verified = await ZEN.verify({ m: inner, s: val["~"] }, sender_pub);
  if (!verified) return null;
  return {
    sender_pub,
    parsed: typeof verified === "string" ? JSON.parse(verified) : verified,
  };
}
...
export async function createChannel(proj_id, chan_id, version = 1, owner_pair, members = [], zen) {
  ...
  for (const [pub, enc] of Object.entries(wrapped)) {
    await zenPut(zen.get(base + "/keys").get(pub), enc, authOpt);
  }
  // Store meta as individual keys (ZEN nodes cannot hold a primitive at root)
  const metaSoul = base + "/meta";
  await zenPut(zen.get(metaSoul).get("pub"),     chan_pair.pub, authOpt);
  await zenPut(zen.get(metaSoul).get("version"), version,       authOpt);
  await zenPut(zen.get(metaSoul).get("since"),   Date.now(),    authOpt);
...
export async function inviteMember(proj_id, chan_id, version = 1, owner_pair, member_pub, zen) {
  ...
  const base      = "zacp/" + owner_pair.pub + "/" + proj_id + "/" + chan_id;
  await zenPut(
    zen.get(base + "/keys").get(member_pub),
    wrapped[member_pub],
...
export async function rotateChanKey(proj_id, chan_id, old_version = 1, owner_pair, remaining_members = [], zen) {
  ...
  const metaSoul = base + "/meta";
  await zenPut(zen.get(metaSoul).get("pub"),     chan_pair.pub, authOpt);
  await zenPut(zen.get(metaSoul).get("version"), new_version,  authOpt);
  await zenPut(zen.get(metaSoul).get("since"),   Date.now(),   authOpt);
...
export async function sendDM(recipient_pub, message, sender_pair, zen) {
  ...
  return zenPut(
    zen.get(soul + "/dm/" + recipient_pub).get(key),
    val,
