...
export function explain(hook, ns) {
  ...
  if (actual) {
    return `acme.sh validates through ${configured}, but ${actual} holds the zone — the challenge is being written where nothing reads it`;
  }
  return `acme.sh validates through ${configured}; the zone's nameservers are ${ns.join(", ")}`;
}
