      ...
      if (r.held) {
        ...
        if (landed) {
          Radix.map(landed, function durable(val, k) {
            if (r.held[k] === val) { delete r.held[k]; }
          });
        }
        if (r.held[key] === data) { delete r.held[key]; }
      }
    ...
    r.read = function (key, cb, o, DBG) {
      ...
      g.get = function (err, disk, info) {
        ...
        if (!o.more) {
          ...
          // Routing to the root file is the same "it never had a chance" case
          // as an empty listing: the directory does not name a file covering
          // this key, so the read looked in the one file used when nothing is
          // listed, found nothing, and that was final.
          //
          // Only while a flush is in flight, though. That is when the directory
          // is still moving and a file that would cover the key is about to be
          // published -- and it is the difference between this and a key that
          // simply does not exist. Asking again for every miss costs the wait
          // on every lookup of an absent key: it took this test from 6.7s to
          // 12.0s a run, which is not a price a read for something that is not
          // there should pay.
          if (
            nothing &&
            r.find.stale &&
            (r.find.stale() || (file === opt.code.from && r.busy()))
          ) {
