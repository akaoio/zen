    ...
    r.read = function (key, cb, o, DBG) {
      ...
      g.get = function (err, disk, info) {
        ...
        if (!o.more) {
          ...
          // Routing to the root file is the same "it never had a chance" case as
          // an empty listing, and far more common: the directory is not empty,
          // it just does not name a file covering this key yet, because the
          // flush that will publish one has not got there. The read then looks
          // in the one file used when nothing is listed, finds nothing, and
          // that was final.
          if (
            nothing &&
            r.find.stale &&
            (r.find.stale() || file === opt.code.from)
          ) {
