...
function Radisk(opt) {
  ...
  r.save = function (key, data, cb, tag, DBG) {
    ...
    s.mix = function (err, disk) {
      ...
      disk.Q = cb ? [cb] : [];
      // Keep the pending flush reachable so a read that finds nothing can ask
      // for it now instead of waiting out the batch.
      disk.flush = s.write;
  ...
  // Bring forward any pending flush that could hold this key.
  //
  // A read that comes back empty may simply be early: the data is merged in
  // memory but the file carrying it has not been written, so the directory
  // cannot route to it yet. Waiting out the batch window is what makes that gap
  // 250ms wide and occasionally much wider. Asking the flush to happen now
  // closes it, and costs nothing when there is nothing pending.
  r.hurry = function (key) {
    var f, disk, any;
    for (f in r.disk) {
      if (!Object.prototype.hasOwnProperty.call(r.disk, f) || f > key) {
        continue;
      }
      disk = r.disk[f];
      if (disk && disk.Q && disk.flush) {
        clearTimeout(disk.to);
        disk.to = null;
        any = 1;
        disk.flush();
      }
