  ...
  r.save = function (key, data, cb, tag, DBG) {
    ...
    s.find = function (file) {
      ...
      if ((tmp = r.writing[file])) {
        ...
        // The queue is per file, and that is only enough while the key still
        // routes to this file. A split publishes new file names as it goes, so
        // mid-flush the directory can start sending this key somewhere that has
        // never heard of it -- and the value is sitting in the queue of the file
        // it used to belong to. Record it in the index a read falls back on, the
        // way the other parking path does, so the answer does not depend on the
        // route staying still.
    ...
    s.ack = function (err, ok) {
      if (r.held) {
        // Everything this flush wrote is durable now, not just the key whose
        // save happened to start the timer. The others returned early -- their
        // callback went on the queue and their own `s.ack` never ran -- so
        // their entry stayed in this index for the life of the process. On a
        // relay that is every value ever written, held in memory for ever, and
        // it sits outside the graph so no eviction reaches it.
        //
        // Drop only what this flush actually wrote. A key written again while
        // the flush was in flight is parked with a newer value, and that one is
        // acknowledged and not yet durable -- which is the whole reason this
        // index exists.
        var landed = d;
        if (landed) {
          Radix.map(landed, function durable(val, k) {
            if (r.held[k] === val) {
              delete r.held[k];
            }
          });
