    ...
    function again() {
      fs.rename(oldPath, newPath, function (err) {
        ...
        if (
          ...
          // Windows fails the rename while something else holds the
          // destination open -- a concurrent read of the very file being
          // replaced. That is transient: the reader lets go in moments.
          //
          // Unlinking the destination to force the rename through turns a
          // transient conflict into a hole. For as long as it takes to retry,
          // the file simply does not exist, and every read of it comes back
          // empty and *stays* wrong, because nothing asks a second time. Retry
          // the rename instead and leave the old copy in place: stale for a few
          // milliseconds beats absent.
          (globalThis.__RENAMEBUSY = globalThis.__RENAMEBUSY || []).push(
            String(newPath).split(/[\\/]/).pop().slice(-22) + "#" + retry,
          );
          if (++retry > max) {
            // Only once retrying is clearly hopeless, fall back to the old
            // behaviour rather than failing the write outright.
            return fs.unlink(newPath, function () {
              fs.rename(oldPath, newPath, function (err2) {
                cb(err2 || err);
              });
            });
          }
          return setTimeout(again, 5 * retry);
        }
        cb(err);
      });
    }
