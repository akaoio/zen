  ...
  root.on("get", function (msg) {
    ...
    dare(
      ...
      function (err, data, info) {
        ...
        // Single-key reads too. The window that makes an empty answer wrong
        // has nothing to do with whether one field or a whole node was asked
        // for, and leaving them out left `test/rad/flush-read.js` -- which
        // reads exactly one field right after an acknowledgement -- still
        // exposed to it.
        if (!info.more && !graph && !err && "string" == typeof soul) {
          ...
          //
          // Measured on Windows: gating this on "a flush is in flight" skipped
          // 178 rungs a run to ask 33 times -- and those 33 rescued 29 reads
          // that would otherwise have been answered "not there" forever. The
          // gate was skipping the ones that mattered, because idle at the
          // instant of a rung says nothing about whether the directory has
          // caught up. So ask regardless.
          //
          // The cost is up to four extra reads for a soul that genuinely does
          // not exist, spread over one batching window, and nothing at all for
