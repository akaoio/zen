    ...
    dare(
      ...
      function (err, data, info) {
        ...
        if (!info.more && !graph && !err && "string" == typeof soul && !o.atom) {
          ...
          // Twice, and early. The directory catches up 5 to 112ms behind the
          // acknowledgement in the samples, so the first ask is timed to cover
          // that; the second covers a whole batching window for the slower
          // cases. Asking only once at the far end is too late to be useful --
          // it did retrieve the node, but by then a caller may have written
          // over what it was waiting for.
          setTimeout(askAgain, Math.max(19, ((opt.until || 250) >> 1) - 9));
          setTimeout(askAgain, (opt.until || 250) + 99);
        }
