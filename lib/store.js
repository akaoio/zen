...
Zen.on("create", function (root) {
  ...
  // when heap usage exceeds opt.maxMemoryMB (default: no cap).
  if (opt.maxMemoryMB && typeof process !== "undefined" && process.memoryUsage) {
    var maxBytes = opt.maxMemoryMB * 1024 * 1024;
    var evictTimer = setInterval(function () {
      var used = process.memoryUsage().heapUsed;
      if (used < maxBytes * 0.8) { return; } // under 80% — nothing to do
      var graph = root.graph;
      if (!graph) { return; }
      var souls = Object.keys(graph);
      if (!souls.length) { return; }
      // Build (soul, lastAccess) pairs — use _.rad timestamp as proxy for access time.
      var pairs = [];
      for (var i = 0; i < souls.length; i++) {
        var soul = souls[i];
        var chain = (root.next || {})[soul];
        // Only evict souls confirmed as persisted on disk (_.rad is set by store.js
        // after a successful radisk read/write) and not currently being written.
        if (chain && chain._ && chain._.rad && !(dare.disk && dare.disk[soul])) {
          pairs.push([soul, chain._.rad]);
        }
      }
      if (!pairs.length) { return; }
      // Sort coldest (oldest access) first and evict up to 20%.
      pairs.sort(function (a, b) { return a[1] - b[1]; });
      var limit = Math.max(1, Math.ceil(pairs.length * 0.2));
      var evicted = 0;
      for (var j = 0; j < limit; j++) {
        delete graph[pairs[j][0]];
        evicted++;
      }
      opt.log && opt.log(
        "ZEN: evicted", evicted, "souls from memory.",
        "Heap:", Math.round(used / 1024 / 1024) + "MB /", opt.maxMemoryMB + "MB limit."
      );
      root.on("memory/evict", { count: evicted, heapMB: Math.round(used / 1024 / 1024) });
    }, 30 * 1000); // every 30 s
