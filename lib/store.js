...
Zen.on("create", function (root) {
  ...
  // opt.maxMemoryMB = false disables the feature entirely.
  if (opt.maxMemoryMB !== false && typeof process !== "undefined" && process.memoryUsage) {
    var __os = (function () { try { return require("os"); } catch (e) { return null; } }());
    // Thresholds (both conditions trigger eviction — whichever is hit first):
    //   minFreeMemRatio : free/total < 10 %  (default)
    //   minFreeMemBytes : free < 200 MB       (default)
    var minFreeRatio = opt.minFreeMemRatio !== undefined ? opt.minFreeMemRatio : 0.10;
    var minFreeBytes = opt.minFreeMemBytes !== undefined ? opt.minFreeMemBytes : (200 * 1024 * 1024);
    ...
    var evictTimer = setInterval(function () {
      ...
      opt.log && opt.log(
        ...
        "System free:", Math.round(free / 1024 / 1024) + "MB",
        "(" + Math.round(ratio * 100) + "% of " + Math.round(total / 1024 / 1024) + "MB)"
      );
