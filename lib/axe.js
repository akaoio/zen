...
function start(root) {
  ...
  (function registerPeersRoute() {
    var server = opt.web;
    if (!server || !server.on) {
      return;
    }
    var route = (server.route = server.route || {});
    route.peers = function (req, res) {
      var seen = {};
      var peers = [];

      function add(url) {
        if (!url || !/^wss?:\/\//.test(url) || seen[url]) {
          return;
        }
        seen[url] = 1;
        peers.push(url);
      ...
      Object.keys(axe.up || {}).forEach(function (pid) {
        var peer = axe.up[pid];
        add(peer && peer.url);
      });

      Object.keys(opt.peers || {}).forEach(function (id) {
