...
// Include the existing AST header from the main project
#include "../../../src/include/zen/core/ast.h"

// We need to compile with our implementation, not the stub
// So we'll include the source file (for testing purposes)
...
void test_ast_node_creation(void)
{
    ...
    assert(literal->identifier == NULL);
    assert(literal->operator == NULL);
    ...
    assert(binary != NULL);
    assert(binary->type == AST_BINARY_OP);
...
/**
 * @brief Test recursive freeing with complex tree
 */
...
int main(void)
{
    printf("Running AST tests (using existing header structure)...\n\n");
