...
bool logic_system_init(void) {
    ...
    g_logic_system->theorems = memory_alloc(g_logic_system->theorem_capacity * sizeof(ZenTheorem*));
    ...
    AST_T* excluded_middle_ast = ast_new_logical_quantifier(QUANTIFIER_UNIVERSAL, "P", NULL, p_or_not_p);
    
    ZenAxiom* excluded_middle = logic_axiom_create(
    ...
    AST_T* non_contradiction_ast = ast_new_logical_quantifier(QUANTIFIER_UNIVERSAL, "P", NULL, not_contradiction);
    
    ZenAxiom* non_contradiction = logic_axiom_create(
        "law_of_non_contradiction", 
        non_contradiction_ast
    );
...
static AST_T* value_to_logical_ast(const Value* value) {
    ...
    if (strcmp(type_str, "quantifier") == 0) {
        ...
        if (!quantifier_type || !variable || !body) return NULL;
        ...
        if (!conn_type) return NULL;
        
        int c_type = CONNECTIVE_AND;  /* default */
        const char* conn_str = conn_type->as.string->data;
        
        if (strcmp(conn_str, "and") == 0) c_type = CONNECTIVE_AND;
        else if (strcmp(conn_str, "or") == 0) c_type = CONNECTIVE_OR;
        else if (strcmp(conn_str, "not") == 0) c_type = CONNECTIVE_NOT;
        else if (strcmp(conn_str, "implies") == 0) c_type = CONNECTIVE_IMPLIES;
        else if (strcmp(conn_str, "iff") == 0) c_type = CONNECTIVE_IFF;
        ...
        if (args && args->type == VALUE_ARRAY) {
            ...
            AST_T* result = ast_new_logical_predicate(name->as.string->data, 
                                                     arg_asts, arg_count);
        ...
        bool bound = is_bound && is_bound->type == VALUE_BOOLEAN && 
                    is_bound->as.boolean;
...
Value* logic_check_godel_sentence(const AST_T* ast) {
    ...
    if (ast->type == AST_LOGICAL_CONNECTIVE && 
        ast->connective_type == CONNECTIVE_NOT) {
        AST_T* inner = ast->connective_right;
        if (inner && inner->type == AST_LOGICAL_PREDICATE &&
            inner->predicate_name && 
    ...
    if (ast->type == AST_LOGICAL_QUANTIFIER &&
        ast->quantifier_type == QUANTIFIER_UNIVERSAL) {
...
static char* ast_step_dump(const AST_T* ast) {
    ...
    switch (ast->type) {
        ...
        case AST_FUNCTION_CALL: {
            ...
            if (result) {
                snprintf(result, strlen(ast->function_call_name) + 50, 
                        "FunctionCall(%s, %zu args)", ast->function_call_name, ast->function_call_arguments_size);
            }
            return result;
        }
        ...
        case AST_LOGICAL_PREDICATE: {
            ...
            if (result) {
                snprintf(result, strlen(name) + 50, "Predicate(%s, %zu args)", name, ast->predicate_args_size);
            }
            return result;
        }
        ...
        case AST_LOGICAL_CONNECTIVE: {
            ...
            switch (ast->connective_type) {
                case CONNECTIVE_AND: conn_name = "∧"; break;
                case CONNECTIVE_OR: conn_name = "∨"; break;
                case CONNECTIVE_IMPLIES: conn_name = "→"; break;
                case CONNECTIVE_IFF: conn_name = "↔"; break;
                case CONNECTIVE_NOT: conn_name = "¬"; break;
            }
        ...
        case AST_MATHEMATICAL_INEQUALITY: {
            ...
            switch (ast->inequality_type) {
                case INEQUALITY_LT: op = "<"; break;
                case INEQUALITY_LE: op = "≤"; break;
                case INEQUALITY_GT: op = ">"; break;
                case INEQUALITY_GE: op = "≥"; break;
            }
            char* result = memory_alloc(50);
            if (result) {
        ...
        case AST_MATHEMATICAL_FUNCTION: {
            ...
            char* result = memory_alloc(strlen(name) + 50);
            if (result) {
                snprintf(result, strlen(name) + 50, "MathFunction(%s, %zu args)", name, ast->math_function_args_size);
            }
            return result;
        }
        ...
        case AST_BINARY_OP: {
            ...
            switch (ast->operator_type) {
                case 1: op_name = "+"; break;
                case 2: op_name = "-"; break;
                case 3: op_name = "*"; break;
                case 4: op_name = "/"; break;
                case 5: op_name = "="; break;  // ZEN uses = for comparison
                case 6: op_name = "!="; break;
                case 7: op_name = "<"; break;
                case 8: op_name = ">"; break;
                case 9: op_name = "<="; break;
                case 10: op_name = ">="; break;
                default: op_name = "op"; break;
            }
            char* result = memory_alloc(50);
            if (result) {
        ...
        case AST_VARIABLE_DEFINITION: {
            const char* name = ast->variable_definition_variable_name ? ast->variable_definition_variable_name : "?";
            char* result = memory_alloc(strlen(name) + 30);
            if (result) {
        ...
        case AST_FUNCTION_DEFINITION: {
            ...
            char* result = memory_alloc(strlen(name) + 50);
            if (result) {
                snprintf(result, strlen(name) + 50, "FuncDef(%s, %zu params)", name, ast->function_definition_args_size);
            }
            return result;
        }
...
Value* logic_theorem_define(Value** args, size_t argc) {
    ...
    if (!logic_system_ensure_initialized()) {
        return error_new_with_code("Failed to initialize logic system", ZEN_LOGIC_ERROR_SYSTEM_NOT_INIT);
    }
    ...
    if (args[1]->type != VALUE_OBJECT) {
        return error_new_with_code("Theorem statement must be AST object", ZEN_LOGIC_ERROR_INVALID_THEOREM);
    }
    ...
    if (!statement_ast) {
        return error_new_with_code("Failed to convert Value to AST", ZEN_LOGIC_ERROR_INVALID_THEOREM);
    }
    ...
    if (g_logic_system->theorem_count >= g_logic_system->theorem_capacity) {
        ...
        ZenTheorem** new_theorems = memory_realloc(g_logic_system->theorems, 
                                                  new_capacity * sizeof(ZenTheorem*));
        if (!new_theorems) {
            logic_theorem_free(theorem);
            return error_new_with_code("Failed to expand theorem array", ZEN_LOGIC_ERROR_MEMORY_ALLOC);
        }
...
Value* logic_proof_verify(Value** args, size_t argc) {
    ...
    if (args[0]->type != VALUE_STRING) {
        return error_new_with_code("Theorem name must be a string", ZEN_LOGIC_ERROR_INVALID_THEOREM);
    }
    ...
    if (step_count == 0) {
        if (is_new_proof) logic_proof_free(proof);
        return error_new_with_code("Proof must have at least one step", ZEN_LOGIC_ERROR_INVALID_PROOF);
    }
    ...
    for (size_t i = 0; i < step_count; i++) {
        ...
        if (step_ast) {
            ...
            if (step && step->type == VALUE_OBJECT) {
                Value* justification_val = object_get(step->as.object, "justification");
    ...
    if (is_new_proof) {
        if (g_logic_system->proof_count >= g_logic_system->proof_capacity) {
            ...
            ZenProof** new_proofs = memory_realloc(g_logic_system->proofs, 
                                                  new_capacity * sizeof(ZenProof*));
            if (new_proofs) {
...
Value* logic_axiom_add(Value** args, size_t argc) {
    ...
    if (args[1]->type != VALUE_OBJECT) {
        return error_new_with_code("Axiom statement must be AST object", ZEN_LOGIC_ERROR_INVALID_AXIOM);
    }
    ...
    if (statement_obj->type == VALUE_OBJECT) {
        ...
        Value* type_val = object_get(statement_obj->as.object, "type");
        Value* content_val = object_get(statement_obj->as.object, "content");
        
        if (type_val && type_val->type == VALUE_STRING) {
            const char* stmt_type = type_val->as.string->data;
            
            if (strcmp(stmt_type, "proposition") == 0 && content_val && content_val->type == VALUE_STRING) {
                statement_ast = ast_new_logical_proposition(content_val->as.string->data);
            } else if (strcmp(stmt_type, "quantifier") == 0) {
                Value* quantifier_val = object_get(statement_obj->as.object, "quantifier");
                Value* variable_val = object_get(statement_obj->as.object, "variable");
                Value* domain_val = object_get(statement_obj->as.object, "domain");
                
                if (quantifier_val && variable_val && domain_val &&
                    quantifier_val->type == VALUE_STRING && variable_val->type == VALUE_STRING && domain_val->type == VALUE_STRING) {
                    statement_ast = ast_new_logical_quantifier(
                        quantifier_val->as.string->data,
                        variable_val->as.string->data,
                        domain_val->as.string->data
                    );
                }
            }
        }
    ...
    if (g_logic_system->axiom_count >= g_logic_system->axiom_capacity) {
        ...
        ZenAxiom** new_axioms = memory_realloc(g_logic_system->axioms, 
                                              new_capacity * sizeof(ZenAxiom*));
        if (!new_axioms) {
            logic_axiom_free(axiom);
            return error_new_with_code("Failed to expand axiom array", ZEN_LOGIC_ERROR_MEMORY_ALLOC);
        }
...
Value* logic_axiom_validate(Value** args, size_t argc) {
    ...
    if (args[0]->type != VALUE_STRING) {
        return error_new_with_code("Axiom identifier must be a string", ZEN_LOGIC_ERROR_INVALID_AXIOM);
    }
    ...
    if (!axiom) {
        ...
        return error_new_with_code("Axiom not found and not a valid identifier", ZEN_LOGIC_ERROR_INVALID_AXIOM);
    }
    
    // Use the axiom's AST statement for consistency checking
    if (!axiom->statement_ast) {
        return error_new_with_code("Axiom has invalid AST structure", ZEN_LOGIC_ERROR_INVALID_AXIOM);
    }
    ...
    for (size_t i = 0; i < g_logic_system->axiom_count && is_consistent; i++) {
        ...
        if (!existing || !existing->statement_ast || !axiom->statement_ast) continue;
...
Value* logic_proof_step(Value** args, size_t argc) {
    ...
    if (args[1]->type != VALUE_OBJECT || args[2]->type != VALUE_OBJECT) {
        return error_new_with_code("Step statement and justification must be AST objects", ZEN_LOGIC_ERROR_INVALID_PROOF);
    }
    ...
    AST_T** new_justifications = memory_realloc(proof->justification_asts, new_count * sizeof(AST_T*));
    ...
    if (!step_ast) {
        if (is_new_proof) logic_proof_free(proof);
        return error_new_with_code("Failed to convert step statement to AST", ZEN_LOGIC_ERROR_INVALID_PROOF);
    }
    
    if (!justification_ast) {
        ast_free(step_ast);
        if (is_new_proof) logic_proof_free(proof);
        return error_new_with_code("Failed to convert justification to AST", ZEN_LOGIC_ERROR_INVALID_PROOF);
    }
...
Value* logic_system_stats(Value** args, size_t argc) {
    ...
    if (!stats_obj) {
        return error_new_with_code("Failed to create statistics object", ZEN_LOGIC_ERROR_MEMORY_ALLOC);
    }
    ...
    object_set(stats_obj, "total_verifications", value_new_number((double)g_logic_system->total_verifications));
    
    double avg_verification_time = 0.0;
    if (g_logic_system->total_verifications > 0) {
        avg_verification_time = g_logic_system->total_verification_time / g_logic_system->total_verifications;
    }
...
Value* logic_entails(Value** args, size_t argc) {
    ...
    if (!premises_ast || !conclusion_ast) {
        if (premises_ast) ast_free(premises_ast);
        if (conclusion_ast) ast_free(conclusion_ast);
    ...
    if (premises_ast->type == AST_LOGICAL_PROPOSITION && 
        ...
        // Check if they're the same proposition
        entails = (strcmp(premises_ast->logical_data.proposition, 
                         conclusion_ast->logical_data.proposition) == 0);
    }
...
Value* logic_modus_ponens(Value** args, size_t argc) {
    if (argc != 2) {
        return error_new("logic_modus_ponens() requires exactly 2 arguments: conditional and antecedent");
    }
    ...
    if (!conditional_ast || !antecedent_ast) {
        if (conditional_ast) ast_free(conditional_ast);
        if (antecedent_ast) ast_free(antecedent_ast);
        return error_new("Failed to convert arguments to logical AST");
    }
    
    Value* result = NULL;
    
    // Check if conditional is an implication (A → B)
    if (conditional_ast->type == AST_LOGICAL_CONNECTIVE &&
        strcmp(conditional_ast->logical_data.connective.operator, "implies") == 0) {
        
        AST_T* left = conditional_ast->logical_data.connective.left;
        AST_T* right = conditional_ast->logical_data.connective.right;
        
        // Check if antecedent matches the left side of implication
        if (left && antecedent_ast->type == AST_LOGICAL_PROPOSITION &&
            left->type == AST_LOGICAL_PROPOSITION &&
            strcmp(left->logical_data.proposition, antecedent_ast->logical_data.proposition) == 0) {
            
            // Return the consequent (right side)
            if (right && right->type == AST_LOGICAL_PROPOSITION) {
                Value* consequent_obj = value_new_object();
                object_set(consequent_obj, "type", value_new_string("proposition"));
                object_set(consequent_obj, "content", value_new_string(right->logical_data.proposition));
                result = consequent_obj;
            } else {
                result = error_new("Invalid consequent in conditional");
            }
        } else {
            result = error_new("Antecedent does not match conditional premise");
        }
    } else {
...
Value* logic_modus_tollens(Value** args, size_t argc) {
    if (argc != 2) {
        return error_new("logic_modus_tollens() requires exactly 2 arguments: conditional and negated consequent");
    }
    ...
    if (!conditional_ast || !negated_consequent_ast) {
        if (conditional_ast) ast_free(conditional_ast);
        if (negated_consequent_ast) ast_free(negated_consequent_ast);
        return error_new("Failed to convert arguments to logical AST");
    }
    ...
    if (conditional_ast->type == AST_LOGICAL_CONNECTIVE &&
        ...
        // Check if negated consequent matches negation of right side
        if (right && negated_consequent_ast->type == AST_LOGICAL_CONNECTIVE &&
            strcmp(negated_consequent_ast->logical_data.connective.operator, "not") == 0) {
            
            AST_T* negated_part = negated_consequent_ast->logical_data.connective.left;
            if (negated_part && right->type == AST_LOGICAL_PROPOSITION &&
                negated_part->type == AST_LOGICAL_PROPOSITION &&
                strcmp(right->logical_data.proposition, negated_part->logical_data.proposition) == 0) {
                
                // Return negation of antecedent (left side)
                if (left && left->type == AST_LOGICAL_PROPOSITION) {
                    Value* negated_antecedent_obj = value_new_object();
                    ...
                    Value* antecedent_obj = value_new_object();
                    object_set(antecedent_obj, "type", value_new_string("proposition"));
                    object_set(antecedent_obj, "content", value_new_string(left->logical_data.proposition));
                    object_set(negated_antecedent_obj, "operand", antecedent_obj);
                    
                    result = negated_antecedent_obj;
                } else {
                    result = error_new("Invalid antecedent in conditional");
                }
            } else {
                result = error_new("Negated consequent does not match conditional consequent");
            }
        } else {
            result = error_new("Second argument must be negation of consequent");
        }
    } else {
        result = error_new("First argument must be a conditional (implication)");
    }
