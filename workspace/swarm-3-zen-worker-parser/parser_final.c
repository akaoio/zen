...
{
    printf("DEBUG: parser_parse_variable_definition called\n");
    ...
    printf("DEBUG: About to parse expression, current_token='%s' type=%d\n", 
...
AST_T* parser_parse_variable(parser_T* parser, scope_T* scope)
{
    ...
    printf("DEBUG parser_parse_variable: token_value='%s', next_token_type=%d\n", 
           token_value, parser->current_token->type);
    
    // Check if this is a function call (has arguments after ID)
    // But NOT if the next token is a binary operator
    bool is_binary_op = parser_is_binary_operator(parser->current_token->type);
    bool has_args = (parser->current_token->type == TOKEN_STRING || 
                     parser->current_token->type == TOKEN_ID ||
                     parser->current_token->type == TOKEN_NUMBER ||
                     parser->current_token->type == TOKEN_TRUE ||
                     parser->current_token->type == TOKEN_FALSE ||
    ...
    printf("DEBUG: is_binary_op=%d, has_args=%d\n", is_binary_op, has_args);
    printf("DEBUG: TOKEN_PLUS constant = %d, current_token_type = %d, precedence = %d\n", 
           TOKEN_PLUS, parser->current_token->type, parser_get_precedence(parser->current_token->type));
    
    // CRITICAL FIX: The fundamental issue is that the '+' token is being lost
    // from the token stream. As a workaround, be more conservative about 
    // when to treat identifiers as function calls.
    
    // Only treat as function call if:
    // 1. Next token is NOT a binary operator, AND  
    // 2. Next token looks like function arguments, AND
    // 3. We're clearly not at end of statement
    if (!is_binary_op && has_args && 
        parser->current_token->type != TOKEN_NEWLINE &&
        parser->current_token->type != TOKEN_EOF &&
        parser->current_token->type != TOKEN_DEDENT &&
        parser->current_token->type != TOKEN_COMMA) {
        
        // ADDITIONAL CHECK: If next token is an ID that could be a variable name
        // in an expression context, be extra cautious
        if (parser->current_token->type == TOKEN_ID) {
            printf("DEBUG: Next token is ID '%s' - treating '%s' as VARIABLE to be safe\n", 
                   parser->current_token->value, token_value);
        } else {
            printf("DEBUG: Creating AST_FUNCTION_CALL for '%s'\n", token_value);
            return parser_parse_function_call(parser, scope);
        }
    }
    
    printf("DEBUG: Creating AST_VARIABLE for '%s'\n", token_value);
