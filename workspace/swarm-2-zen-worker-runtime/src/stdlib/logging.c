...
const char* logging_level_string(ZenLogLevel level) {
    switch (level) {
        case ZEN_LOG_DEBUG: return "DEBUG";
        case ZEN_LOG_INFO:  return "INFO ";
        case ZEN_LOG_WARN:  return "WARN ";
        case ZEN_LOG_ERROR: return "ERROR";
        default:            return "UNKN ";
    }
}
...
static void log_output(ZenLogLevel level, const char* context, const char* message) {
    ...
    if (context) {
        fprintf(stderr, "[%s] [%s] [%s] %s\n", 
                timestamp, logging_level_string(level), context, message);
    } else {
        fprintf(stderr, "[%s] [%s] %s\n", 
...
static char* log_format_message(const char* format_str, Value** args, size_t argc) {
    ...
    for (size_t i = 1; i < argc && offset < ZEN_MAX_LOG_MESSAGE_SIZE - 1; i++) {
        if (args[i]) {
            ...
            offset += snprintf(buffer + offset, ZEN_MAX_LOG_MESSAGE_SIZE - offset - 1, " %s", str_repr);
        }
    }
