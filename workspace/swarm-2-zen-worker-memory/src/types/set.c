...
Value* set_remove(Value* set, const Value* item) {
    ...
    // Since object_delete doesn't return success status, we verify by checking
    // if the item still exists after deletion attempt
    char* check_key_str = value_to_string(item);
    if (!check_key_str) {
        memory_free(key);
        return value_new_boolean(false);
    }
    
    Value* still_exists = object_get(set_data->hash_table, check_key_str);
    bool was_removed = (still_exists == NULL);
    
    memory_free(check_key_str);
    memory_free(key);
