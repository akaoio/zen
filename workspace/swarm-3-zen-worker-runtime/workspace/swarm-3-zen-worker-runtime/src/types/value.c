    ...
    switch (value->type) {
        ...
        case VALUE_PRIORITY_QUEUE: {
            ...
            if (size_val && size_val->type == VALUE_NUMBER) {
                ...
                if (result) {
                    sprintf(result, "<priority_queue size=%.0f>", size_val->as.number);
                }
                value_unref(size_val);
                return result ? result : strdup("<priority_queue>");
            }
            if (size_val) value_unref(size_val);
            return strdup("<priority_queue>");
        }
        default:
...
char* value_to_string_safe(const Value* value) {
    ...
    if (!result) {
        ...
        switch (value->type) {
            ...
            case VALUE_UNDECIDABLE: return strdup("undecidable");
