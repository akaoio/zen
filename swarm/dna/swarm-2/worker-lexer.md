...
Created: 2025-08-07T09:53:31.595Z
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "What I'm doing" target-files)
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-2-zen-worker-lexer "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-2-zen-worker-lexer`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-2-zen-worker-lexer "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-2-zen-worker-lexer "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-2-zen-worker-lexer")
    ...
    local coord_task=$(node task.js create swarm-2-zen-worker-lexer "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-2-zen-worker-lexer "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-2-zen-worker-lexer, part of swarm-2.
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Brief description of what you're about to do" file1 file2 | grep "Created task:" | cut -d' ' -f3)
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-2-zen-worker-lexer "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Brief description of your task" file1.c file2.h | grep "Created task:" | cut -d' ' -f3)

# Example:
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Implement lexer_scan_number function" src/core/lexer.c src/include/zen/core/lexer.h | grep "Created task:" | cut -d' ' -f3)
```
...
node task.js list --active | grep swarm-2-zen-worker-lexer

# List completed tasks
node task.js list --completed | grep swarm-2-zen-worker-lexer
```
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-lexer "Implement lexer_scan_string function" src/core/lexer.c | grep "Created task:" | cut -d' ' -f3)
