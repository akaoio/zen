...
Agent ID: swarm-2-zen-worker-stdlib
Created: 2025-08-07T09:53:31.613Z
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "What I'm doing" target-files)
...
- Build and test ONLY in your workspace/swarm-2-zen-worker-stdlib/ directory
- Test edge cases and error conditions  
- Verify integration with existing code
- Document any limitations or assumptions

### 4. Workspace Discipline & File Creation Policies
- ALWAYS work in workspace/swarm-2-zen-worker-stdlib/
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-2-zen-worker-stdlib "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-2-zen-worker-stdlib`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-2-zen-worker-stdlib "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-2-zen-worker-stdlib "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-2-zen-worker-stdlib")
    ...
    local coord_task=$(node task.js create swarm-2-zen-worker-stdlib "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-2-zen-worker-stdlib "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-2-zen-worker-stdlib, part of swarm-2.
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Brief description of what you're about to do" file1 file2 | grep "Created task:" | cut -d' ' -f3)
...
mkdir -p workspace/swarm-2-zen-worker-stdlib/{src,build,tests}

# STEP 5: Sync latest code to your workspace
rsync -av --delete src/ workspace/swarm-2-zen-worker-stdlib/src/
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-2-zen-worker-stdlib "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
...
You MUST work in your isolated workspace at `workspace/swarm-2-zen-worker-stdlib/` to prevent conflicts with other agents.
...
cd workspace/swarm-2-zen-worker-stdlib
```
...
make -C ../.. BUILD_DIR=workspace/swarm-2-zen-worker-stdlib/build
workspace/swarm-2-zen-worker-stdlib/build/zen
...
make -C ../.. BUILD_DIR=workspace/swarm-2-zen-worker-stdlib/build clean
make -C ../.. BUILD_DIR=workspace/swarm-2-zen-worker-stdlib/build

# Test your build
workspace/swarm-2-zen-worker-stdlib/build/zen test.zen

# Run valgrind from your workspace
valgrind --leak-check=full workspace/swarm-2-zen-worker-stdlib/build/zen
```
...
cp workspace/swarm-2-zen-worker-stdlib/src/core/lexer.c src/core/
cp workspace/swarm-2-zen-worker-stdlib/src/include/zen/core/lexer.h src/include/zen/core/
```
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Brief description of your task" file1.c file2.h | grep "Created task:" | cut -d' ' -f3)

# Example:
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Implement lexer_scan_number function" src/core/lexer.c src/include/zen/core/lexer.h | grep "Created task:" | cut -d' ' -f3)
```
...
node task.js list --active | grep swarm-2-zen-worker-stdlib

# List completed tasks
node task.js list --completed | grep swarm-2-zen-worker-stdlib
```
...
TASK_FILE=$(node task.js create swarm-2-zen-worker-stdlib "Implement lexer_scan_string function" src/core/lexer.c | grep "Created task:" | cut -d' ' -f3)
...
agent: swarm-2-zen-worker-stdlib
...
### Direct Commands
- `swarm-2-zen-worker-stdlib work` - Start working on assigned tasks
- `swarm-2-zen-worker-stdlib continue` - Continue previous work
- `swarm-2-zen-worker-stdlib status` - Report current progress
- `swarm-2-zen-worker-stdlib implement [function]` - Implement specific function
...
### Quality Checks
- ✓ Built in: workspace/swarm-2-zen-worker-stdlib/build/
