...
Agent ID: swarm-test-zen-worker-types
Created: 2025-08-05T15:36:30.876Z
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Description" files... | grep "Created task:" | cut -d' ' -f3)
...
- Build and test ONLY in your workspace/swarm-test-zen-worker-types/ directory
- Test edge cases and error conditions  
- Verify integration with existing code
- Document any limitations or assumptions

### 4. Workspace Discipline
- ALWAYS work in workspace/swarm-test-zen-worker-types/
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-test-zen-worker-types "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-test-zen-worker-types`
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-test-zen-worker-types "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-test-zen-worker-types "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-test-zen-worker-types, part of swarm-test.
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Brief description of what you're about to do" file1 file2 | grep "Created task:" | cut -d' ' -f3)
...
mkdir -p workspace/swarm-test-zen-worker-types/{src,build,tests}

# STEP 5: Sync latest code to your workspace
rsync -av --delete src/ workspace/swarm-test-zen-worker-types/src/
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-test-zen-worker-types "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
...
You MUST work in your isolated workspace at `workspace/swarm-test-zen-worker-types/` to prevent conflicts with other agents.
...
cd workspace/swarm-test-zen-worker-types
```
...
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-types/build
workspace/swarm-test-zen-worker-types/build/zen
...
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-types/build clean
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-types/build

# Test your build
workspace/swarm-test-zen-worker-types/build/zen test.zen

# Run valgrind from your workspace
valgrind --leak-check=full workspace/swarm-test-zen-worker-types/build/zen
```
...
cp workspace/swarm-test-zen-worker-types/src/core/lexer.c src/core/
cp workspace/swarm-test-zen-worker-types/src/include/zen/core/lexer.h src/include/zen/core/
```
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Brief description of your task" file1.c file2.h | grep "Created task:" | cut -d' ' -f3)

# Example:
TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Implement lexer_scan_number function" src/core/lexer.c src/include/zen/core/lexer.h | grep "Created task:" | cut -d' ' -f3)
```
...
node task.js list --active | grep swarm-test-zen-worker-types

# List completed tasks
node task.js list --completed | grep swarm-test-zen-worker-types
```
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-types "Implement lexer_scan_string function" src/core/lexer.c | grep "Created task:" | cut -d' ' -f3)
...
agent: swarm-test-zen-worker-types
...
### Direct Commands
- `swarm-test-zen-worker-types work` - Start working on assigned tasks
- `swarm-test-zen-worker-types continue` - Continue previous work
- `swarm-test-zen-worker-types status` - Report current progress
- `swarm-test-zen-worker-types implement [function]` - Implement specific function
...
### Quality Checks
- ✓ Built in: workspace/swarm-test-zen-worker-types/build/
