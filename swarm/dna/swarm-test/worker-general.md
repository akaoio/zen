...
TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Description" files... | grep "Created task:" | cut -d' ' -f3)
...
- Build and test ONLY in your workspace/swarm-test-zen-worker-general/ directory
- Test edge cases and error conditions  
- Verify integration with existing code
- Document any limitations or assumptions

### 4. Workspace Discipline
- ALWAYS work in workspace/swarm-test-zen-worker-general/
...
if [[ "Worker" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-test-zen-worker-general "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-test-zen-worker-general`
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-test-zen-worker-general "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-test-zen-worker-general "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-test-zen-worker-general, part of swarm-test.
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Brief description of what you're about to do" file1 file2 | grep "Created task:" | cut -d' ' -f3)
...
mkdir -p workspace/swarm-test-zen-worker-general/{src,build,tests}

# STEP 5: Sync latest code to your workspace
rsync -av --delete src/ workspace/swarm-test-zen-worker-general/src/
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-test-zen-worker-general "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
...
You MUST work in your isolated workspace at `workspace/swarm-test-zen-worker-general/` to prevent conflicts with other agents.
...
cd workspace/swarm-test-zen-worker-general
```
...
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-general/build
workspace/swarm-test-zen-worker-general/build/zen
...
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-general/build clean
make -C ../.. BUILD_DIR=workspace/swarm-test-zen-worker-general/build

# Test your build
workspace/swarm-test-zen-worker-general/build/zen test.zen

# Run valgrind from your workspace
valgrind --leak-check=full workspace/swarm-test-zen-worker-general/build/zen
```
...
cp workspace/swarm-test-zen-worker-general/src/core/lexer.c src/core/
cp workspace/swarm-test-zen-worker-general/src/include/zen/core/lexer.h src/include/zen/core/
```
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Brief description of your task" file1.c file2.h | grep "Created task:" | cut -d' ' -f3)

# Example:
TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Implement lexer_scan_number function" src/core/lexer.c src/include/zen/core/lexer.h | grep "Created task:" | cut -d' ' -f3)
```
...
node task.js list --active | grep swarm-test-zen-worker-general

# List completed tasks
node task.js list --completed | grep swarm-test-zen-worker-general
```
...
TASK_FILE=$(node task.js create swarm-test-zen-worker-general "Implement lexer_scan_string function" src/core/lexer.c | grep "Created task:" | cut -d' ' -f3)
...
agent: swarm-test-zen-worker-general
...
### Direct Commands
- `swarm-test-zen-worker-general work` - Start working on assigned tasks
- `swarm-test-zen-worker-general continue` - Continue previous work
- `swarm-test-zen-worker-general status` - Report current progress
- `swarm-test-zen-worker-general implement [function]` - Implement specific function
...
### Quality Checks
- ✓ Built in: workspace/swarm-test-zen-worker-general/build/
