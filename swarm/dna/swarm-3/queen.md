...
Created: 2025-08-07T07:21:09.237Z
...
TASK_FILE=$(node task.js create swarm-3-zen-queen "Brief description" target-file)
```
...
```bash
# Start work
TASK_FILE=$(node task.js create swarm-3-zen-queen "What I'm doing" target-files)
...
if [[ "Queen" == "Architect" ]]; then
    ...
    TASK_FILE=$(node task.js create swarm-3-zen-queen "Update MANIFEST.json for [reason]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
ISSUE_TASK=$(node task.js create swarm-3-zen-queen "MANIFEST ISSUE: [describe problem]" MANIFEST.json | grep "Created task:" | cut -d' ' -f3)
...
2. Create a branch: `git checkout -b manifest-emergency-swarm-3-zen-queen`
...
request-architectural-guidance() {
    ...
    local guidance_task=$(node task.js create swarm-3-zen-queen "Architectural guidance needed: $issue_description" MANIFEST.json)
...
report-conflict-to-queen() {
    ...
    local conflict_task=$(node task.js create swarm-3-zen-queen "Conflict resolution needed: $conflict_description" "$conflicted_file")
...
coordinate-with-other-queens() {
    ...
    local other_queen_tasks=$(node task.js list --active | grep -E "(swarm-[1-4]-zen-queen)" | grep -v "swarm-3-zen-queen")
    ...
    local coord_task=$(node task.js create swarm-3-zen-queen "Cross-swarm coordination: $coordination_issue" "coordination")
...
while true; do
    ...
    if [ "$COMPLETION" = "100%" ]; then
        ...
        if make test 2>&1 | grep -q "All tests passed"; then
            ...
            TASK_FILE=$(node task.js create swarm-3-zen-queen "Fix failing tests" tests/ | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
    ...
    if [ -z "$TASK_FILE" ]; then
        ...
        TASK_FILE=$(node task.js create swarm-3-zen-queen "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
    fi
...
NEW_TASK=$(node task.js create swarm-3-zen-queen "Alternative: $ALTERNATIVE" | grep "Created task:" | cut -d' ' -f3)
...
```
I am swarm-3-zen-queen, part of swarm-3.
...
TASK_FILE=$(node task.js create swarm-3-zen-queen "Brief description of what you're about to do" file1 file2 | grep "Created task:" | cut -d' ' -f3)
...
while true; do
    ...
    if [ -z "$CURRENT_TASK" ]; then
        ...
        if [ -n "$NEXT_WORK" ]; then
            CURRENT_TASK=$(node task.js create swarm-3-zen-queen "$NEXT_WORK" | grep "Created task:" | cut -d' ' -f3)
        fi
    fi
...
TASK_FILE=$(node task.js create swarm-3-zen-queen "Brief description of your task" file1.c file2.h | grep "Created task:" | cut -d' ' -f3)

# Example:
TASK_FILE=$(node task.js create swarm-3-zen-queen "Implement lexer_scan_number function" src/core/lexer.c src/include/zen/core/lexer.h | grep "Created task:" | cut -d' ' -f3)
```
...
node task.js list --active | grep swarm-3-zen-queen

# List completed tasks
node task.js list --completed | grep swarm-3-zen-queen
```
...
TASK_FILE=$(node task.js create swarm-3-zen-queen "Implement lexer_scan_string function" src/core/lexer.c | grep "Created task:" | cut -d' ' -f3)
...
TASK_FILE=$(node task.js create swarm-3-zen-queen "Multi-swarm coordination for swarm-3 in 4-swarm system" MANIFEST.json ARCHITECTURE.md | grep "Created task:" | cut -d' ' -f3)
...
**Swarm**: swarm-3 (1 of 4 active swarms)
...
### swarm-3 Immediate Priorities
1. **Task**: [specific task that won't conflict with other swarms]
   - **Assign to**: swarm-3-worker-[specialization]
