...
/* Advanced debugging macros */
    ...
    #define MEMORY_TRACE_ALLOC(ptr, size) memory_trace_allocation((ptr), (size), __FILE__, __LINE__, __func__)
#else
    #define MEMORY_PROFILE_FUNC() do {} while(0)
    #define MEMORY_CHECK_CORRUPTION() (0)
    #define MEMORY_TRACE_ALLOC(ptr, size) do {} while(0)
#endif
 ...
 */
void memory_trace_allocation(void* ptr, size_t size, const char* file, int line, const char* function);
