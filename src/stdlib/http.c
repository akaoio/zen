...
bool http_headers_add(HttpHeaders* headers, const char* key, const char* value) {
    ...
    if (!headers->keys[headers->count] || !headers->values[headers->count]) {
        if (headers->keys[headers->count]) memory_free(headers->keys[headers->count]);
        if (headers->values[headers->count]) memory_free(headers->values[headers->count]);
        return false;
    }
...
Value* http_create_response_object(long status, const char* body, const HttpHeaders* headers) {
    ...
    if (body && strlen(body) > 0) {
        ...
        if (!body_val || body_val->type == VALUE_ERROR) {
            ...
            if (body_val) value_unref(body_val);
 ...
 */
static Value* http_perform_request(const char* method, const Value* url_value, 
                                 const Value* data_value, const Value* headers_value) {
    // Validate URL
        ...
        if (req_headers) {
            for (size_t i = 0; i < req_headers->count; i++) {
                ...
                snprintf(header_line, sizeof(header_line), "%s: %s", 
                        req_headers->keys[i], req_headers->values[i]);
                curl_headers = curl_slist_append(curl_headers, header_line);
            }
    ...
    if (curl_result == CURLE_OK) {
        ...
        char error_msg[512];
        snprintf(error_msg, sizeof(error_msg), "HTTP request failed: %s", curl_easy_strerror(curl_result));
