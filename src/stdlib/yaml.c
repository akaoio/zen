...
static bool emit_yaml_value_safe(yaml_emitter_t* emitter, const Value* value, YamlRefTracker* tracker);
static bool emit_yaml_sequence_safe(yaml_emitter_t* emitter, const Value* array, YamlRefTracker* tracker);
static bool emit_yaml_mapping_safe(yaml_emitter_t* emitter, const Value* object, YamlRefTracker* tracker);
...
static bool emit_yaml_value_safe(yaml_emitter_t* emitter, const Value* value, YamlRefTracker* tracker) {
    ...
    if (tracker->depth >= YAML_MAX_DEPTH) {
        yaml_event_t event;
        yaml_scalar_event_initialize(&event, NULL, NULL, (unsigned char*)"\"[Max Depth Reached]\"", -1, 1, 1, YAML_DOUBLE_QUOTED_SCALAR_STYLE);
        return yaml_emitter_emit(emitter, &event);
    }
    ...
    if (value->type == VALUE_OBJECT || value->type == VALUE_ARRAY) {
        if (yaml_ref_tracker_contains(tracker, value)) {
            yaml_event_t event;
            yaml_scalar_event_initialize(&event, NULL, NULL, (unsigned char*)"\"[Circular Reference]\"", -1, 1, 1, YAML_DOUBLE_QUOTED_SCALAR_STYLE);
            return yaml_emitter_emit(emitter, &event);
        }
...
static bool yaml_anchor_map_add(YamlAnchorMap* map, const char* name, Value* value) {
    ...
    if (map->count >= map->capacity) {
        ...
        YamlAnchor* new_anchors = memory_realloc(map->anchors, 
                                                 sizeof(YamlAnchor) * new_capacity);
        if (!new_anchors) {
            return false;
        }
