 ...
 */
static RegexCache* add_to_cache(const char* pattern, pcre2_code_8* compiled_pattern, pcre2_match_data_8* match_data) {
    ...
    if (cache_size >= cache_capacity) {
        size_t new_capacity = cache_capacity == 0 ? REGEX_CACHE_INITIAL_CAPACITY : cache_capacity * 2;
        ...
        if (new_capacity > REGEX_CACHE_MAX_SIZE) {
            ...
            memmove(pattern_cache, &pattern_cache[cache_size - keep_count], 
        ...
        if (new_capacity != cache_capacity) {
            RegexCache* new_cache = memory_realloc(pattern_cache, new_capacity * sizeof(RegexCache));
            if (!new_cache) {
                return NULL;
            }
...
static RegexCache* compile_pattern_cached(const char* pattern) {
    ...
    pcre2_code_8* compiled_pattern = pcre2_compile_8(
        (PCRE2_SPTR8)pattern,
...
static Value* get_pcre2_error_message(int error_code, PCRE2_SIZE error_offset, const char* pattern) {
    ...
    snprintf(full_error, sizeof(full_error), 
        ...
        error_buffer, error_offset, pattern ? pattern : "null");
...
static Value* validate_string_input(const Value* value, const char* param_name) {
    ...
    if (value->type != VALUE_STRING) {
        char error_msg[128];
        snprintf(error_msg, sizeof(error_msg), "%s must be a string, got %s", 
                param_name, value_type_name(value->type));
...
Value* regex_match(const Value* text_value, const Value* pattern_value) {
    ...
    if (!regex_cache) {
        int error_code;
        PCRE2_SIZE error_offset;
        pcre2_compile_8((PCRE2_SPTR8)pattern, PCRE2_ZERO_TERMINATED, ZEN_PCRE2_OPTIONS, &error_code, &error_offset, NULL);
        return get_pcre2_error_message(error_code, error_offset, pattern);
    }
    
    // Perform match
    int match_count = pcre2_match_8(
        regex_cache->compiled_pattern,
        (PCRE2_SPTR8)text,
        strlen(text),
        0,                           // Start offset
        0,                           // Options
...
Value* regex_replace(const Value* text_value, const Value* pattern_value, const Value* replacement_value) {
    ...
    int result_code = pcre2_substitute_8(
        regex_cache->compiled_pattern,
        (PCRE2_SPTR8)text,
        PCRE2_ZERO_TERMINATED,
        0,                              // Start offset
    ...
    if (result_code < 0) {
        ...
        if (result_code == PCRE2_ERROR_NOMEMORY) {
            ...
            result_code = pcre2_substitute_8(
                regex_cache->compiled_pattern,
                (PCRE2_SPTR8)text,
                PCRE2_ZERO_TERMINATED,
                0,
...
Value* regex_replace_stdlib(Value** args, size_t argc) {
    if (argc != 3) {
        return error_new("regexReplace requires exactly 3 arguments: text, pattern, and replacement");
    }
