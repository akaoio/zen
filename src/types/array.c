...
{
    if (!array1 || array1->type != VALUE_ARRAY || !array2 || array2->type != VALUE_ARRAY) {
        return NULL;
    }
    ...
    for (size_t i = 0; i < source->length; i++) {
        ...
        if (test_result) {
            ...
            case VALUE_NUMBER:
                ...
                break;
            case VALUE_STRING:
                is_truthy = (test_result->as.string && test_result->as.string->length > 0);
                break;
            case VALUE_NULL:
                is_truthy = false;
                break;
