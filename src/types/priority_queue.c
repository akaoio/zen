...
ZenPriorityQueue *datastructures_priority_queue_create(void)
{
    ...
    queue->items =
        (ZenPriorityItem *)memory_alloc(sizeof(ZenPriorityItem) * PRIORITY_QUEUE_INITIAL_CAPACITY);
    if (!queue->items) {
        memory_free(queue);
        return NULL;
    }
...
bool datastructures_priority_queue_ensure_capacity(ZenPriorityQueue *queue, size_t min_capacity)
{
    ...
    ZenPriorityItem *new_items =
        (ZenPriorityItem *)memory_realloc(queue->items, sizeof(ZenPriorityItem) * new_capacity);
 ...
 */
size_t datastructures_priority_queue_parent(size_t index) { return (index - 1) / 2; }
 ...
 */
size_t datastructures_priority_queue_left_child(size_t index) { return 2 * index + 1; }
 ...
 */
size_t datastructures_priority_queue_right_child(size_t index) { return 2 * index + 2; }
...
void datastructures_priority_queue_heap_down(ZenPriorityQueue *queue, size_t index)
{
    while (true) {
        ...
        if (right < queue->length &&
            queue->items[right].priority > queue->items[largest].priority) {
            largest = right;
        }
