...
token_T* lexer_get_next_token(lexer_T* lexer)
{
    ...
    while (lexer->c != '\0' && lexer->i < content_length)
    {
        ...
        while ((lexer->c == ' ' || lexer->c == '\t' || lexer->c == '\n') ||
            ...
            if (lexer->c == '/' && lexer->i + 1 < content_length && lexer->contents[lexer->i + 1] == '/') {
        ...
        if (isdigit(lexer->c)) {
            ...
            while (lookahead < strlen(lexer->contents) && 
                   (isdigit(lexer->contents[lookahead]) || 
                    lexer->contents[lookahead] == '.' || 
                    lexer->contents[lookahead] == '_' ||
                    lexer->contents[lookahead] == 'e' ||
                    lexer->contents[lookahead] == 'E' ||
                    lexer->contents[lookahead] == '+' ||
                    lexer->contents[lookahead] == '-')) {
...
int lexer_keyword_type(const char* value)
{
    ...
    if (strcmp(value, "undecidable") == 0) return TOKEN_UNDECIDABLE;
    ...
    if (strcmp(value, "get") == 0) return TOKEN_GET;
    if (strcmp(value, "put") == 0) return TOKEN_PUT;
    ...
    if (strcmp(value, "when") == 0) return TOKEN_WHEN;
    if (strcmp(value, "unless") == 0) return TOKEN_UNLESS;
    if (strcmp(value, "whenever") == 0) return TOKEN_WHENEVER;
    if (strcmp(value, "until") == 0) return TOKEN_UNTIL;
    if (strcmp(value, "during") == 0) return TOKEN_DURING;
    if (strcmp(value, "throughout") == 0) return TOKEN_THROUGHOUT;
    if (strcmp(value, "otherwise") == 0) return TOKEN_OTHERWISE;
...
{
    // Core ZEN type names
    if (strcmp(value, "integer") == 0) return true;
    if (strcmp(value, "int") == 0) return true;
    if (strcmp(value, "float") == 0) return true;
    if (strcmp(value, "number") == 0) return true;
    if (strcmp(value, "decimal") == 0) return true;
    if (strcmp(value, "string") == 0) return true;
    if (strcmp(value, "text") == 0) return true;
    if (strcmp(value, "boolean") == 0) return true;
    if (strcmp(value, "bool") == 0) return true;
    if (strcmp(value, "array") == 0) return true;
    if (strcmp(value, "list") == 0) return true;
    if (strcmp(value, "object") == 0) return true;
    if (strcmp(value, "record") == 0) return true;
    if (strcmp(value, "dict") == 0) return true;
...
{
    if (!contents || !config) return NULL;
...
token_T* lexer_collect_enhanced_number(lexer_T* lexer)
{
    ...
    if (!value || !clean_value) {
        if (value) memory_free(value);
        if (clean_value) memory_free(clean_value);
        return NULL;
    }
    ...
    #define APPEND_CLEAN_CHAR(ch) do { \
    ...
    while (isdigit(lexer->c) || (lexer->c == '.' && !has_dot) || lexer->c == '_')
    {
        ...
        if (lexer->c == '.') {
            ...
            if (value_len == 0 || clean_len == 0) {
                if (lexer->i + 1 >= strlen(lexer->contents) || 
                    (!isdigit(lexer->contents[lexer->i + 1]) && lexer->contents[lexer->i + 1] != '_')) {
                    break;
                }
            }
            has_dot = 1;
...
{
    if (!lexer || !lexer->contents) return NULL;
...
token_T* lexer_collect_unicode_id(lexer_T* lexer)
{
    ...
    while (lexer_is_unicode_identifier_char(lexer->c))
    {
        ...
        if ((unsigned char)lexer->c < 128) {
            ...
            if ((first_byte & 0xE0) == 0xC0) bytes_to_read = 2;      // 110xxxxx
            else if ((first_byte & 0xF0) == 0xE0) bytes_to_read = 3; // 1110xxxx  
            else if ((first_byte & 0xF8) == 0xF0) bytes_to_read = 4; // 11110xxx
...
{
    // ASCII letters, digits, and underscore (fast path)
    if (ch < 128) {
    ...
    if (ch >= 0x00C0 && ch <= 0x024F) {
        ...
        if (ch >= 0x00C0 && ch <= 0x00D6) return true;  // À-�
        if (ch >= 0x00D8 && ch <= 0x00F6) return true;  // Ø-ö
        if (ch >= 0x00F8 && ch <= 0x024F) return true;  // ø-ɏ
    }
    ...
    if (ch >= 0x05D0 && ch <= 0x05EA) return true;  // Hebrew letters
    if (ch >= 0x05F0 && ch <= 0x05F2) return true;  // Hebrew ligatures
    
    // Arabic (U+0600-U+06FF): Arabic, Persian, Urdu
    if (ch >= 0x0621 && ch <= 0x063A) return true;  // Arabic letters
    if (ch >= 0x0641 && ch <= 0x064A) return true;  // More Arabic letters
    if (ch >= 0x0660 && ch <= 0x0669) return true;  // Arabic-Indic digits
    if (ch >= 0x0671 && ch <= 0x06D3) return true;  // Extended Arabic
    if (ch >= 0x06FA && ch <= 0x06FF) return true;  // Arabic supplement
    
    // Thai (U+0E00-U+0E7F)
    if (ch >= 0x0E01 && ch <= 0x0E30) return true;  // Thai consonants and vowels
    if (ch >= 0x0E32 && ch <= 0x0E33) return true;  // Thai vowels
    if (ch >= 0x0E40 && ch <= 0x0E46) return true;  // Thai vowels and signs
    if (ch >= 0x0E50 && ch <= 0x0E59) return true;  // Thai digits
    
    // Hiragana (U+3040-U+309F): Japanese
    if (ch >= 0x3041 && ch <= 0x3096) return true;  // Hiragana letters
    
    // Katakana (U+30A0-U+30FF): Japanese
    if (ch >= 0x30A1 && ch <= 0x30FA) return true;  // Katakana letters
    
    // CJK Unified Ideographs (U+4E00-U+9FFF): Chinese, Japanese, Korean
    if (ch >= 0x4E00 && ch <= 0x9FFF) return true;  // Main CJK block
    ...
    if (ch >= 0x0531 && ch <= 0x0556) return true;  // Armenian capital letters
    if (ch >= 0x0561 && ch <= 0x0587) return true;  // Armenian lowercase letters
    
    // Georgian (U+10A0-U+10FF)
    if (ch >= 0x10A0 && ch <= 0x10C5) return true;  // Georgian capital letters
    if (ch >= 0x10D0 && ch <= 0x10FA) return true;  // Georgian letters
    
    // Devanagari (U+0900-U+097F): Hindi, Sanskrit, Marathi
    if (ch >= 0x0905 && ch <= 0x0939) return true;  // Devanagari letters
    if (ch >= 0x0958 && ch <= 0x0961) return true;  // Additional letters
    if (ch >= 0x0966 && ch <= 0x096F) return true;  // Devanagari digits
    
    // Bengali (U+0980-U+09FF)
    if (ch >= 0x0985 && ch <= 0x098C) return true;  // Bengali vowels
    if (ch >= 0x098F && ch <= 0x0990) return true;  // Bengali vowels
    if (ch >= 0x0993 && ch <= 0x09A8) return true;  // Bengali consonants
    if (ch >= 0x09AA && ch <= 0x09B0) return true;  // Bengali consonants
    if (ch >= 0x09B2 && ch <= 0x09B9) return true;  // Bengali consonants
    if (ch >= 0x09E6 && ch <= 0x09EF) return true;  // Bengali digits
    
    // Tamil (U+0B80-U+0BFF)
    if (ch >= 0x0B85 && ch <= 0x0B8A) return true;  // Tamil vowels
    if (ch >= 0x0B8E && ch <= 0x0B90) return true;  // Tamil vowels
    if (ch >= 0x0B92 && ch <= 0x0B95) return true;  // Tamil vowels
    if (ch >= 0x0B99 && ch <= 0x0B9A) return true;  // Tamil consonants
    if (ch >= 0x0B9C && ch <= 0x0B9F) return true;  // Tamil consonants
    if (ch >= 0x0BA3 && ch <= 0x0BA4) return true;  // Tamil consonants
    if (ch >= 0x0BA8 && ch <= 0x0BAA) return true;  // Tamil consonants
    if (ch >= 0x0BAE && ch <= 0x0BB9) return true;  // Tamil consonants
    if (ch >= 0x0BE6 && ch <= 0x0BEF) return true;  // Tamil digits
