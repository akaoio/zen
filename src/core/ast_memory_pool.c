...
static bool ast_pool_init_single(ASTMemoryPool* pool, ASTPoolType pool_type, size_t initial_capacity);
...
bool ast_pool_expand(ASTMemoryPool* pool) {
    ...
    AST_POOL_DEBUG("Expanding pool: nodes_per_chunk=%zu, node_size=%zu, chunk_size=%zu, sizeof(AST_T)=%zu",
                   pool->nodes_per_chunk, pool->node_size, chunk_size, sizeof(AST_T));
    ...
    AST_POOL_DEBUG("Expanded pool: chunk_size=%zu nodes, total_chunks=%zu", 
                   pool->nodes_per_chunk, pool->total_chunks);
...
void ast_pool_debug_status(void) {
    ...
    for (int i = 0; i < AST_POOL_COUNT; i++) {
        ...
        size_t current_usage = pool->allocations - pool->deallocations;
        double efficiency = pool->allocations > 0 ? 
            ((double)pool->allocations / (pool->malloc_calls * pool->nodes_per_chunk)) * 100.0 : 0.0;
        
        printf("%s_NODE_POOL: %zu/%zu used (%.1f%% efficiency)\n",
               pool_names[i], current_usage, 
               pool->total_chunks * pool->nodes_per_chunk, efficiency);
        printf("  Chunks: %zu, Malloc calls: %zu, Peak usage: %zu\n",
               pool->total_chunks, pool->malloc_calls, pool->peak_usage);
...
void ast_pool_global_init(void) {
    ast_pool_manager_init(&g_ast_pool_manager);
}
...
void ast_pool_global_cleanup(void) {
    ast_pool_manager_cleanup(&g_ast_pool_manager);
}
...
void ast_pool_defragment(ASTMemoryPool* pool) {
    ...
    for (size_t chunk_idx = 0; chunk_idx < pool->chunk_count; chunk_idx++) {
        ...
        char* current = (char*)chunk;
        size_t nodes_in_chunk = (chunk_idx == pool->chunk_count - 1) ? 
                               pool->nodes_per_chunk : pool->nodes_per_chunk;
