...
parser_T* parser_new(lexer_T* lexer)
{
    ...
    // Initialize AST memory pools for better parsing performance
    ast_pool_global_init();  // Global pool initialization
...
AST_T* parser_parse_statement(parser_T* parser, scope_T* scope)
{
    switch (parser->current_token->type)
    {
        ...
        case TOKEN_GET: return parser_parse_file_get(parser, scope);
...
AST_T* parser_parse_statements(parser_T* parser, scope_T* scope)
{
           ...
           parser->current_token->type != TOKEN_DEDENT)
    {
        ...
        if (ast_statement) {
            ...
            compound->compound_statements = memory_realloc(
...
AST_T* parser_parse_function_definition(parser_T* parser, scope_T* scope)
{
    ...
    while (parser->current_token->type == TOKEN_ID || 
           parser->current_token->type == TOKEN_SPREAD) {
        ...
        if (parser->current_token->type == TOKEN_SPREAD) {
            ...
            if (has_rest_param) {
                ...
                LOG_ERROR(LOG_CAT_PARSER, "Multiple rest parameters not allowed in function definition");
                break;
            }
...
AST_T* parser_parse_primary_expr(parser_T* parser, scope_T* scope)
{
    ...
    while (parser->current_token->type == TOKEN_LBRACKET || 
        ...
        if (parser->current_token->type == TOKEN_LBRACKET) {
            ...
            snprintf(array_access->property_name, 32, "[%ld]", 
...
AST_T* parser_parse_id_or_object(parser_T* parser, scope_T* scope)
{
    ...
    char* original_name = memory_strdup(parser->current_token->value); // Must duplicate before parser_eat
    ...
    if (token_count >= 4) {
        ...
        if (tokens[0]->type == TOKEN_ID &&
            (tokens[1]->type == TOKEN_STRING || tokens[1]->type == TOKEN_NUMBER || 
             ...
             tokens[1]->type == TOKEN_NULL || tokens[1]->type == TOKEN_UNDECIDABLE || tokens[1]->type == TOKEN_ID) &&
            tokens[2]->type == TOKEN_COMMA &&
            tokens[3]->type == TOKEN_ID) {
        ...
        if (tokens[0]->type == TOKEN_ID &&
            (tokens[1]->type == TOKEN_STRING || tokens[1]->type == TOKEN_NUMBER ||
             tokens[1]->type == TOKEN_TRUE || tokens[1]->type == TOKEN_FALSE ||
             tokens[1]->type == TOKEN_NULL || tokens[1]->type == TOKEN_UNDECIDABLE || tokens[1]->type == TOKEN_LBRACKET)) {
    ...
    if (parser->current_token->type == TOKEN_ID) {
        ...
        do {
            ...
            if (import_node->import_names_size >= names_capacity) {
                names_capacity *= 2;
                import_node->import_names = memory_realloc(
                    import_node->import_names,
                    sizeof(char*) * names_capacity
                );
            }
