...
static bool config_safe_strcpy(char* dest, const char* src, size_t max_len) {
    ...
    if (src_len >= max_len) return false;
...
bool config_load_from_env(void) {
    ...
    if (env_value) {
        config_safe_strcpy(g_config.config_file, env_value, 
                          sizeof(g_config.config_file));
    }
    
    // Load module path
    env_value = getenv(ZEN_ENV_MODULE_PATH);
    if (env_value) {
        config_safe_strcpy(g_config.module_path, env_value, 
                          sizeof(g_config.module_path));
    }
...
bool config_set(const char* key, const char* value) {
    ...
    // Path configuration
        ...
        return config_safe_strcpy(g_config.module_path, value, 
                                sizeof(g_config.module_path));
    }
    else if (strcmp(key, "config_file") == 0) {
        return config_safe_strcpy(g_config.config_file, value, 
                                sizeof(g_config.config_file));
    }
