...
void ast_free(AST_T* ast)
{
    ...
    // Free string values
    if (ast->string_value) memory_free(ast->string_value);
    if (ast->variable_definition_variable_name) memory_free(ast->variable_definition_variable_name);
    if (ast->function_definition_name) memory_free(ast->function_definition_name);
    if (ast->variable_name) memory_free(ast->variable_name);
    if (ast->function_call_name) memory_free(ast->function_call_name);
    ...
    if (ast->file_ref_target_file) memory_free(ast->file_ref_target_file);
    if (ast->file_ref_property_path) memory_free(ast->file_ref_property_path);

    // Free object keys
    if (ast->object_keys) {
        for (size_t i = 0; i < ast->object_size; i++) {
            if (ast->object_keys[i]) memory_free(ast->object_keys[i]);
        }
    ...
    if (ast->import_names) {
        for (size_t i = 0; i < ast->import_names_size; i++) {
            if (ast->import_names[i]) memory_free(ast->import_names[i]);
        }
        memory_free(ast->import_names);
    }

    // Recursively free child AST nodes
    if (ast->variable_definition_value) ast_free(ast->variable_definition_value);
    if (ast->function_definition_body) ast_free(ast->function_definition_body);
    if (ast->left) ast_free(ast->left);
    if (ast->right) ast_free(ast->right);
    if (ast->operand) ast_free(ast->operand);
    if (ast->condition) ast_free(ast->condition);
    if (ast->then_branch) ast_free(ast->then_branch);
    if (ast->else_branch) ast_free(ast->else_branch);
    if (ast->loop_condition) ast_free(ast->loop_condition);
    if (ast->loop_body) ast_free(ast->loop_body);
    if (ast->iterable) ast_free(ast->iterable);
    if (ast->for_body) ast_free(ast->for_body);
    if (ast->return_value) ast_free(ast->return_value);
    if (ast->object) ast_free(ast->object);
    if (ast->try_block) ast_free(ast->try_block);
    if (ast->catch_block) ast_free(ast->catch_block);
    if (ast->exception_value) ast_free(ast->exception_value);
    if (ast->lambda_body) ast_free(ast->lambda_body);

    // Free arrays of AST nodes
    if (ast->function_definition_args) {
        for (size_t i = 0; i < ast->function_definition_args_size; i++) {
            if (ast->function_definition_args[i]) ast_free(ast->function_definition_args[i]);
        }
    ...
    if (ast->array_elements) {
        for (size_t i = 0; i < ast->array_size; i++) {
            if (ast->array_elements[i]) ast_free(ast->array_elements[i]);
        }
    ...
    if (ast->object_values) {
        for (size_t i = 0; i < ast->object_size; i++) {
            if (ast->object_values[i]) ast_free(ast->object_values[i]);
        }
    ...
    if (ast->compound_statements) {
        for (size_t i = 0; i < ast->compound_size; i++) {
            if (ast->compound_statements[i]) ast_free(ast->compound_statements[i]);
        }
    ...
    if (ast->class_methods) {
        for (size_t i = 0; i < ast->class_methods_size; i++) {
            if (ast->class_methods[i]) ast_free(ast->class_methods[i]);
        }
    ...
    if (ast->lambda_args) {
        for (size_t i = 0; i < ast->lambda_args_size; i++) {
            if (ast->lambda_args[i]) ast_free(ast->lambda_args[i]);
        }
